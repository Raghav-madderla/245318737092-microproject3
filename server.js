...
app.post('/AddData', (req,res)=>{			
	db.collection('Brands').save(req.body, (err,result)=>{			
		if(err) return console.log(err)
	res.redirect('/')                        
	})
})
...
app.post('/update', (req,res)=>{			
		db.collection('Brands').find().toArray((err,result)=>{	   			
			...
			db.collection('Brands').findOneAndUpdate({pid:req.body.id},{
...
app.post('/delete',(req,res)=>{
	db.collection('Brands').findOneAndDelete({pid:req.body.id}, (err,result)=>{    
		if(err)
			return console.log(err)
		res.redirect('/')
	})
